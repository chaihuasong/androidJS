...
adb shell "run-as com.termux sh -c '
  ...
  if ! grep -q patch.js \$DEST/index.html; then
    sed -i \"s|<script type=\\\"module\\\"|<script src=\\\"./patch.js?v=14\\\"></script>\\n    <script type=\\\"module\\\"|\" \$DEST/index.html
    echo patched
  else
    # 更新版本号
    sed -i \"s|patch.js?v=[0-9]*|patch.js?v=14|\" \$DEST/index.html
    echo updated
  fi
'"

echo "完成！在浏览器中刷新页面（F5）后生效。"
echo "Console 里看到 [clawbot-patch] v14 loaded 即表示成功。"
