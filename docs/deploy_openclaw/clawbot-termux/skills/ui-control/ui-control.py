...
def dump_ui(retries=3):
    for attempt in range(1, retries + 1):
        # --compressed 减少节点数量，速度更快；超时放宽到 30s
        result = subprocess.run(
            f"uiautomator dump --compressed {UI_XML} 2>/dev/null",
            shell=True, capture_output=True, timeout=30
        )
        time.sleep(0.3)
        xml_str = run(f"cat {UI_XML}", timeout=5)
...
def cmd_key(key):
    ...
    if key not in keycodes:
        ...
        sys.exit(1)
    log(f"按键: {key}")
    shell(f"input keyevent {keycodes[key]}")
    time.sleep(0.4)
    log("OK")


def cmd_screenshot():
    path = "/sdcard/ui-screenshot.png"
