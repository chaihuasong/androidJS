...
tmux new-window -t "$SESSION_NAME" -n tunnel \
  "echo '--- SSH Reverse Tunnel to $CLOUD_SERVER ---'; echo 'Mapping localhost:$GATEWAY_PORT -> $CLOUD_SERVER:$GATEWAY_PORT'; ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o TCPKeepAlive=yes -o ExitOnForwardFailure=yes -R $GATEWAY_PORT:127.0.0.1:$GATEWAY_PORT $CLOUD_SERVER -N; echo 'Tunnel disconnected. Press Enter to retry.'; read"
